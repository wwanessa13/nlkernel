#' Kernel PCA with Laplacian Kernel for Multi-Environment Genomic Prediction
#'
...
#' Default is "env_g_pca_laplacian.xlsx".
#'
#' @details
#' The model implemented is:
#' \deqn{y = Xb + Zg + e}
#' where \eqn{Xb} represents fixed environmental effects and \eqn{Zg} represents
#' the main genomic effect modeled with a Laplacian kernel PCA-derived marker
#' matrix. First, the SNP matrix is transformed using kernel PCA with a
#' Laplacian kernel. Then, the kernel principal components are selected according
...
#'   seed = 123,
#'   save_xlsx = TRUE,
#'   file_name = "env_g_pca_laplacian.xlsx"
#' )
#'
#' results
#' }
...
env_g_pca_laplacian <- function(SNPs, y, IDs, env,
                              EZ = NULL,
                              CV = c("CV1", "CV2", "CV0"),
                              sg = c(0.001, 0.01, 0.05, 0.1),
                              exp_var = 0.90,
  ...
  if (length(GDec_list) == 0) {
    stop("No KPCA Laplacian model was fitted. No valid KPCA components were returned.")
  }
  ...
  for (i in seq_along(GDec_list)) {
    ...
    cat(
      "\nRunning KPCA Laplacian model with sigma =",
      GDec_i$sigma,
      "| nPC =", GDec_i$nPC, "\n"
    )

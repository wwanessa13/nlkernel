...
#' framework implemented in the BGLR package. Environmental effects are fitted as
#' fixed effects, and genomic kernels are expanded from the genotype level to the
#' observation level.
#'
...
#' environmental incidence matrix, genotype incidence matrix, kernels, eigen decompositions,
#' model information, hyperparameters, and training data.
#'
#' @examples
#' \dontrun{
...
#'   bessel_order = 1,
#'   bessel_degree = 2,
#'   file_name = "final_env_laplacian_bessel.rds"
#' )
#' }
#'
#' @export
...
train_final_multig <- function(SNPs,
  ...
  decompose_kernel <- function(K_obs) {
    ...
    eig <- eigen(K_obs, symmetric = TRUE)
  ...
  cat("\nTraining final multi-environment BGLR model using all available data\n")
  ...
  kernel_decompositions <- lapply(observation_kernels, decompose_kernel)
  ...
  model_object <- list(
    ...
    kernel_decompositions = kernel_decompositions,
  ...
  class(model_object) <- "train_final_env"

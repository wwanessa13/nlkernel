#' Multi-Environment Genomic Prediction via Bessel Kernel
#'
#' This function performs genomic prediction across multiple environments by
#' integrating genotype and environmental information using a Bessel Kernel under a main effects framework.
...
#'   }
#' @param degree A numeric vector of degree values for the Bessel kernel. Default is 2 and 3.
#' @param order A numeric vector of order values for the Bessel kernel. Default is 0, 1 and 2.
#' @param sigma A numeric vector of sigma values for the Bessel kernel. Default is 0.1, 0.5 and 1.
...
#' \dontrun{
#' # Example usage:
#' results <- env_g_bessel(SNPs = X, y = data$yield, IDs = data$id,
#'                             env = data$location, CV = "CV1")
#' }
#'
#' @export

env_g_bessel <- function(SNPs, y, IDs, env,
                         EZ = NULL,
                         CV = c("CV1", "CV2", "CV0"),
                         degree = c(2, 3),
                         order = c(0, 1, 2),
  ...
  if (save_xlsx) {
    ...
    if (is.null(file_name)) {
      file_name <- paste0("Bessel_", CV, ".xlsx")
    }
